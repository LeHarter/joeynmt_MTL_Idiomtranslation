...
from joeynmt.transformer_layers import PositionalEncoding, TransformerEncoderLayer, TransformerEncoderLayerSagT5
        ...
        **kwargs,
        ...
        self.layers = nn.ModuleList([
            TransformerEncoderLayer(
                ...
                ff_size=ff_size,
                num_heads=num_heads,
                specialized_head = False,
                dropout=dropout,
                alpha=kwargs.get("alpha", 1.0),
                layer_norm=kwargs.get("layer_norm", "pre"),
                activation=kwargs.get("activation", "relu"),
            ) if layer != special_head_layer else
            TransformerEncoderLayer(
                size=hidden_size,
                ff_size=ff_size,
                num_heads=num_heads,
                specialized_head = True,
                dropout=dropout,
                alpha=kwargs.get("alpha", 1.0),
                layer_norm=kwargs.get("layer_norm", "pre"),
                activation=kwargs.get("activation", "relu"),
    ...
    def forward(
        ...
        **kwargs,
        ...
        for layer in self.layers:
            if layer.specialized_head:
                x,idiom_grouping = layer(x,mask)
            else:
                x = layer(x, mask)

        if self.layer_norm is not None:
            x = self.layer_norm(x)
        return x, idiom_grouping, None
...
class TransformerEncoderSagT5(Encoder):
    """
    Transformer Encoder
    """
