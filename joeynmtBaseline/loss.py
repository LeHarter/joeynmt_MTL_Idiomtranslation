...
class XentLoss(nn.Module):
    ...
    def forward(self, log_probs: Tensor,log_probsIdiom, **kwargs) -> Tensor:
        ...
        #print(log_probsIdiom.shape,kwargs["idiom"].shape)
        #print(log_probsIdiom[0,0,:],kwargs["idiom"][0,0])
        #print(log_probsIdiom[0,-1,:],kwargs["idiom"][0,-1])
        #print(kwargs["trg"][0,-1])
        log_probsIdiom, idioms = self._reshape(log_probsIdiom[:,:,:],kwargs["idiom"])
        # compute loss
        logits = self.criterion(log_probs, targets)
        logitsidiom = self.criterionIdiom(log_probsIdiom,idioms)
