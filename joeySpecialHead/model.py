...
class Model(nn.Module):
    ...
    def forward(self,
        ...
        if return_type == "loss":
            ...
            out, x, att_probs, idiom_grouping = self._encode_decode(**kwargs)

            # compute log probs
            log_probs = F.log_softmax(out, dim=-1)
            log_probsIdiom = F.log_softmax(idiom_grouping,dim=-1)
            # compute batch loss
            ...
            encoder_output, idiom_grouping,encoder_hidden = self._encode(**kwargs)
    ...
    def _encode_decode(
        ...
        **kwargs,
        ...
        :return: decoder outputs
        """
        encoder_output, idiom_grouping,encoder_hidden = self._encode(src=src,
        ...
        return out, x, att, idiom_grouping
        """
        return self._decode(
    ...
    def label_sequence(self,batch):
        encoder_output, idiom_grouping,encoder_hidden = self._encode(src=batch.src,src_length=batch.src_length,src_mask=batch.src_mask)
        labels = torch.argmax(idiom_grouping[0][1:,:],dim=-1)
        #print(tagger_output.shape)
        #return tagger_output
        print(labels)
        print(torch.softmax(idiom_grouping[0][1:,:],dim=-1))
        return labels
...
def build_model(cfg: dict = None,
    ...
    enc_cfg = cfg["encoder"]
    dec_cfg = cfg["decoder"]
    specialhead_cfg = cfg["specialhead"]
    ...
    if enc_cfg.get("type", "recurrent") == "transformer":
        ...
        encoder = TransformerEncoder(
            **enc_cfg,
            emb_size=emb_size,
            special_head_layer=specialhead_cfg.get("layer",0),
