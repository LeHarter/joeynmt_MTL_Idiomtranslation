...
class SubwordNMTTokenizer(BasicTokenizer):
    ...
    def post_process(self,
        ...
        # ensure the string is not empty.
        if not (sequence is not None and len(sequence) > 0):
            print(sequence)
        assert sequence is not None and len(sequence) > 0, sequence
        return sequence
    ...
    def copy_cfg_file(self, model_dir: Path) -> None:
        ...
        #print("\n")
        #print(model_dir)
        #print(self.codes.name)
        #print("\n")
        #output_file = Path("/foo/bar/baz.txt")
        #output_file.parent.mkdir(exist_ok=True, parents=True)
        shutil.copy2(self.codes.name, (model_dir / self.codes.name.split("/")[-1]).as_posix())
...
def build_tokenizer(data_cfg: Dict) -> Dict[str, BasicTokenizer]:
    ...
    tokenizer = {
        ...
        "idiom": _build_tokenizer(data_cfg["idiom"]),
    }
