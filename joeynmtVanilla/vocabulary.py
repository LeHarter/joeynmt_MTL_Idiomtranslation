...
class Vocabulary:
    ...
    def __init__(self, tokens: List[str],idiom=False) -> None:
        """
...
def _build_vocab(cfg: Dict, dataset: BaseDataset = None,idiom=False) -> Vocabulary:
    """
    ...
    vocab = Vocabulary(unique_tokens,idiom)
...
def build_vocab(cfg: Dict,
    ...
    if model_dir is not None and cfg["idiom"].get("voc_file", None) is None:
        assert (model_dir / "bio.txt").is_file()
        cfg["idiom"]["voc_file"] = (model_dir / "bio.txt").as_posix()

    src_vocab = _build_vocab(cfg["src"], dataset,False)
    trg_vocab = _build_vocab(cfg["trg"], dataset,False)
    idiom_vocab = _build_vocab(cfg["idiom"],dataset,True)
    ...
    return src_vocab, trg_vocab, idiom_vocab
