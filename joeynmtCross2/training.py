...
class TrainManager:
    ...
    def __init__(self, model: Model, cfg: dict) -> None:
        ...
        elif self.early_stopping_metric in ["acc", "bleu", "chrf","bertscore"]:  # higher is better
    ...
    def _validate(self, valid_data: Dataset):
        ...
        #print("Gewichte:",self.model.weightedTaggerLogits.weight)
        #print("Scale:",self.model.scaleReducing,torch.sigmoid(self.model.scaleReducing))
        # save checkpoints
...
def train(cfg_file: str, skip_test: bool = False) -> None:
    ...
    # load the data
    src_vocab, trg_vocab, idiom_vocab, train_data, dev_data, test_data = load_data(
        data_cfg=cfg["data"])
    ...
    idiom_vocab.to_file(model_dir / "idiom_vocab.txt")
    if hasattr(train_data.tokenizer[train_data.idiom_lang], "copy_cfg_file"):
        train_data.tokenizer[train_data.idiom].copy_cfg_file(model_dir)
    ...
    for lang in ["en","de"]:
        with open("valid_tokens."+lang,"w",encoding="utf-8") as f:
            for i in range(0,len(dev_data)):
                f.write(str(dev_data.get_item(i,lang=lang))+"\n")
        with open("test_tokens."+lang,"w",encoding="utf-8") as f:
            for i in range(0,len(test_data)):
                f.write(str(test_data.get_item(i,lang=lang))+"\n")
        with open("train_tokens."+lang,"w",encoding="utf-8") as f:
            for i in range(0,len(train_data)):
                f.write(str(train_data.get_item(i,lang=lang))+"\n")
    """
