...
class Model(nn.Module):
    ...
    def _encode_decode(
        ...
        **kwargs,
        ...
        prob_for_idiom = self.softmax(tagger_output)
        prob_for_idiom = (prob_for_idiom[:,:,1:2]+prob_for_idiom[:,:,2:3])
        norm = torch.norm(prob_for_idiom,dim=1).repeat(1,prob_for_idiom.shape[1]).view(prob_for_idiom.shape[0],prob_for_idiom.shape[1],prob_for_idiom.shape[2])
        xmean = torch.mean(prob_for_idiom,dim=1).repeat(1,prob_for_idiom.shape[1]).view(prob_for_idiom.shape[0],prob_for_idiom.shape[1],prob_for_idiom.shape[2])
        meandiff = prob_for_idiom.sub(xmean)
        n_n = torch.stack([meandiff.div(norm).view(prob_for_idiom.shape[0],prob_for_idiom.shape[1])],dim=-1)
        n_n = n_n.repeat([4,1,1]).view(n_n.shape[0],4,n_n.shape[1],n_n.shape[2])
    ...
    def _decode(
        ...
        **_kwargs,
        ...
        if idiom_label_mask == None:
            ...
            xmean = torch.mean(prob_for_idiom,dim=1).repeat(1,prob_for_idiom.shape[1]).view(prob_for_idiom.shape[0],prob_for_idiom.shape[1],prob_for_idiom.shape[2])
            meandiff = prob_for_idiom.sub(xmean)
            n_norm = torch.stack([meandiff.div(norm).view(prob_for_idiom.shape[0],prob_for_idiom.shape[1])],dim=-1)
            idiom_label_mask = n_norm.repeat([4,1,1]).view(n_norm.shape[0],4,n_norm.shape[1],n_norm.shape[2])
