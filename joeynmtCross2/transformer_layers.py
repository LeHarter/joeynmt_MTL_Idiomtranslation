...
class MultiHeadedAttention(nn.Module):
    ...
    def forward(
        ...
        q: Tensor,
        mask: Optional[Tensor] = None,
        idiom_label_mask: Optional[Tensor]=None,
        return_weights: Optional[bool] = None,
    ):
        """
        Computes multi-headed attention.
        ...
        # apply the mask (if we have one)
        # we add a dimension for the heads to it below: [batch_size, 1, 1, key_len]
        if idiom_label_mask is not None:
            #scores = torch.add(scores,idiom_label_mask)
            attention_n = torch.stack([torch.norm(scores,dim=-1)],dim=-1).transpose(-1,-2)
            modifiers = idiom_label_mask.matmul(attention_n).transpose(-1,-2)
            scores = torch.sub(scores,modifiers)
